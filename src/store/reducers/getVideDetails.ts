...
export const getVideoDetails = createAsyncThunk(
    ...
    async (id: string) => {
        const {
            ...
            `${YOUTUBE_DATA_API_URL}/videos?key=${YOUTUBE_API_KEY}&part=snippet,statistics&type=video&id=${id}`
        );
...
const parseData = async (item: {
    ...
    const {
        ...
        `${YOUTUBE_DATA_API_URL}/channels?part=snippet,statistics&id=${item.snippet.channelId}&key=${YOUTUBE_API_KEY}`
    );
