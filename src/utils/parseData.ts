...
import { YOUTUBE_DATA_API_URL, YOUTUBE_API_KEY } from "./constants";
...
export const parseData = async (items: any[]) => {
    try {
        ...
        const { data: { items: channelsData }, } = await axios.get(
            `${YOUTUBE_DATA_API_URL}/channels?part=snippet,contentDetails&id=${channelIds.join(",")}&key=${YOUTUBE_API_KEY}`);
